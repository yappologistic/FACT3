...
function ChatRouteGlobalShortcuts() {
  ...
  const executeCommand = useCallback(
    (command: string | null): boolean => {
        ...
        if (!isPreviewSupportedInRuntime()) {
          toastManager.add(
            stackedThreadToast({
              ...
              description: "Open FACT3 Code in the desktop app to use the in-app preview.",
            }),
          );
          return true;
        }
      ...
      if (
        ...
        if (!routeThreadRef || !previewOpen || !isPreviewSupportedInRuntime()) {
          return false;
        }
        const action =
          command === "preview.refresh"
  ...
  useEffect(() => onAppCommand(executeCommand), [executeCommand]);
