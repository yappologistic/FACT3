...
import { ChatHeader, type WorkspaceView } from "./chat/ChatHeader";
...
function ChatViewContent(props: ChatViewProps) {
  ...
  const rightPanelOpen = workspaceView === "chat" && rightPanelState.isOpen;
  ...
  const archivedEnvironmentIds = useMemo(
    () =>
      workspaceView === "board" && kanbanHistoryOpen && activeProject
        ? [activeProject.environmentId]
        : [],
    [activeProject, kanbanHistoryOpen, workspaceView],
  );
  const archivedThreadSnapshots = useArchivedThreadSnapshots(archivedEnvironmentIds);
  const kanbanArchivedThreads = useMemo<ThreadShell[]>(
    () =>
      activeProject
        ...
        : [],
    [activeProject, archivedThreadSnapshots.snapshots],
  );
  ...
  const handleOpenKanbanDiff = useCallback(
    (thread: ThreadShell, scope: "branch" | "unstaged") => {
    ...
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      >
        ...
        >
          ...
          <ChatHeader
            ...
            onWorkspaceViewChange={(view) => {
              ...
              if (view === "chat") setKanbanHistoryOpen(false);
            }}
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          {workspaceView === "board" && activeProject ? (
            <KanbanBoard
              project={activeProject}
              threads={kanbanThreads}
              archivedThreads={kanbanArchivedThreads}
              archivedThreadsLoading={archivedThreadSnapshots.isLoading}
