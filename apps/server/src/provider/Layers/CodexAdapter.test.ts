...
sessionErrorLayer("CodexAdapterLive session errors", (it) => {
  ...
  it.effect("configures the T3 MCP server when the Codex runtime starts", () => {
    const runtimeFactory = makeRuntimeFactory();
    const threadId = asThreadId("sess-mcp-startup");
    ...
    const layer = Layer.effect(
      ...
      Effect.gen(function* () {
        const codexConfig = decodeCodexSettings({});
        return yield* makeCodexAdapter(codexConfig, {
          environment: { TEST_ENV: "preserved" },
          makeRuntime: runtimeFactory.factory,
        });
      }),
    ).pipe(
...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  it.effect("keeps forwarding events after the session-start caller completes", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;
      ...
      yield* runtime.emit({
        id: asEventId("evt-after-start-caller"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        ...
        threadId: asThreadId("thread-1"),
        turnId: asTurnId("turn-1"),
        itemId: asItemId("msg-after-start-caller"),
        payload: {
  ...
  it.effect("keeps Codex sub-agent start markers active until the child turn completes", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);

      yield* runtime.emit({
        id: asEventId("evt-subagent-start-marker-complete"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        createdAt: "2026-01-01T00:00:00.000Z",
        method: "item/completed",
      ...
      NodeAssert.equal(firstEvent.value.type, "item.updated");
  ...
  it.effect("maps terminal Codex sub-agent markers to completed collaboration items", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);

      yield* runtime.emit({
        id: asEventId("evt-subagent-terminal"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        createdAt: "2026-01-01T00:00:00.000Z",
        method: "item/completed",
  ...
  it.effect("maps synthesized completed sub-agent markers without the upstream schema", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);

      yield* runtime.emit({
        id: asEventId("evt-subagent-synthetic-completed"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        createdAt: "2026-01-01T00:00:00.000Z",
        method: "item/completed",
      ...
      NodeAssert.equal(
        (firstEvent.value.payload.data as { item?: { kind?: string } }).item?.kind,
        "completed",
      );
    }),
  );

  it.effect("preserves synthesized failed sub-agent markers as failures", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);

      yield* runtime.emit({
        id: asEventId("evt-subagent-synthetic-failed"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        createdAt: "2026-01-01T00:00:00.000Z",
        method: "item/completed",
  ...
  it.effect("maps routed child settings to the existing sub-agent item", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);

      yield* runtime.emit({
        id: asEventId("evt-subagent-runtime-settings"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        createdAt: "2026-01-01T00:00:00.000Z",
      ...
      NodeAssert.equal(firstEvent.value.itemId, "spawn_1");
